...
## DEFINE PARAMETERS ##
# consistent for all LSTM models
...
class LSTM(nn.Module):
    ...
    def forward(self,x):
        ...
        out = self.relu(output) # experimental relu layer??? - MOVE THIS AFTER OUTPUT
        out = self.fc(out)
...
# ## TRAIN MODEL ##
# # returns ONLY loss records (no validation)
# def train_lstm(lstm, train_swe_tensors, train_non_swe_tensors, num_epochs, loss_fn, optimizer):
#     # train model - doesn't work for 1 yr of data
#     loss_record = np.zeros(num_epochs)
#     for epoch in range(num_epochs):
#         # training
