## TRAIN AND EVALUATE REGIONAL LSTM MODEL ##
...
func_map_site = { 'get_sites_full' : get_sites_full, 'get_sites_random' : get_sites_random, 'get_sites_latitude' : get_sites_latitude}

## DEFINE RUN ##
# inputs are run name, site selection method, number of sites, year selection method, number of years
run = input("run name: ")
func_sites = input("function for sites (get_sites_full or get_sites_random or get_sites_latitude): ") 
num_sites = int(input("number of train sites: ")) #*2                       # double for testing if testing set is NOT already defined
...
## GET SNOTEL SITES in CO ##
co_snotel = func_map_site[func_sites]('CO', num_sites)


## GET TRAINING/TESTING DATA ##
data = pd.DataFrame(columns=['site_id', 'year','train'])

for i in range(0, len(co_snotel)):
    site_id = co_snotel['site_id'][i]
    start_date = co_snotel['first_wy_date'][i]
    end_date = co_snotel['last_wy_date'][i]
    try:
...
data_test = pd.read_csv('regional_test_years.txt', sep=' ',header=None)
...
## SAVE METADATA  ##
data_train.to_csv('output/'+run+'_train_metadata.csv',sep=' ', index=False)
...
with open("output/"+run+"_normalize.pkl" , 'wb') as file:  
...
torch.save(train_swe_tensors, 'output/'+run+'_train_swe.pt')
torch.save(train_non_swe_tensors, 'output/'+run+'_train_non_swe.pt')
torch.save(test_swe_tensors, 'output/'+run+'_test_swe.pt')
torch.save(test_non_swe_tensors, 'output/'+run+'_test_non_swe.pt')
...
## TRAIN MODEL ##
loss_record, val_record = train_lstm(lstm, train_swe_tensors, train_non_swe_tensors, test_swe_tensors, test_non_swe_tensors)


## SAVE MODEL ##
torch.save(lstm, 'output/'+run+'_lstm.pt')


## EVALUATE LSTM ##
ev_lstm = torch.load('output/'+run+'_lstm.pt', map_location = DEVICE)
ev_test_swe = torch.load('output/'+run + '_test_swe.pt')
ev_test_non_swe = torch.load('output/'+run + '_test_non_swe.pt')

statistics, feature_importance = analyze_results(ev_lstm, data_test, ev_test_swe, ev_test_non_swe, scaler_swe, False)
...
statistics.to_csv('output/'+run+'_statistics.txt',sep=' ',header=None, index=False, index_label=False)